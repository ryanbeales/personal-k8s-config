...
2. **Certificates**: A cert-manager Certificate resource requests a TLS certificate for `router.ryanbeales.com` using Route53 DNS-01 verification. This secret is provisioned locally in the cluster but cannot be applied via the Terraform provider due to provider limitations (it must be configured on OPNsense manually or via the ACME client plugin).
3. **Crossplane Terraform Provider**: A Crossplane `Workspace` executes Terraform HCL using the `browningluke/opnsense` provider to manage Kea DHCP subnets and static reservations.
