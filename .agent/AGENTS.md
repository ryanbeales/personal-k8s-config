...
- **Restarts**: Do NOT add dummy environment variables or meaningless changes to Git to force a pod restart. To restart pods, you may use `kubectl rollout restart` or `kubectl delete pod`, or ask the user to sync/restart the application directly in the ArgoCD UI.
